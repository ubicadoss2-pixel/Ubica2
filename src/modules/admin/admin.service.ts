...
export const listPendingPlacesAdmin = async () => {
...
export const moderatePlaceAdmin = async (placeId: string, status: "PUBLISHED" | "SUSPENDED", actorUserId: string, reason?: string) => {
  const before = await prisma.place.findUnique({ where: { id: placeId } });
  if (!before) throw new Error("Negocio no encontrado");
  ...
  await audit(actorUserId, `PLACE_MODERATION_${status}`, "PLACE", placeId, 
...
export const approveBusinessAdmin = async (placeId: string, actorUserId: string) => {
  return moderatePlaceAdmin(placeId, "PUBLISHED", actorUserId);
};
...
export const moderateEventAdmin = async (eventId: string, status: "ACTIVE" | "REJECTED", actorUserId: string, reason?: string) => {
  const before = await prisma.event.findUnique({ where: { id: eventId } });
  if (!before) throw new Error("Evento no encontrado");
  ...
  await audit(actorUserId, `EVENT_MODERATION_${status}`, "EVENT", eventId, 
    { ...before, moderationReason: reason }, 
    event
  );
...
export const approveEventAdmin = async (eventId: string, actorUserId: string) => {
  return moderateEventAdmin(eventId, "ACTIVE", actorUserId);
};
