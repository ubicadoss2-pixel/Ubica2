...
const uploadBase64Image = async (base64Str: string): Promise<string> => {
  if (base64Str && base64Str.startsWith("data:image/")) {
    const uploadRes = await cloudinary.uploader.upload(base64Str, { folder: "places" });
...
export const createPlace = async (data: CreatePlaceDTO, ownerUserId: string, isAdmin: boolean) => {
  ...
  let uploadedPhotos: { url: string; sortOrder: number }[] = [];
...
export const listPlaces = async (query: any, userId?: string, role?: string) => {
  ...
  if (isSponsored) where.isSponsored = true;
  ...
  const resultsWithRatings = await Promise.all(
    results.map(async (item) => {
      const stats = await getEntityRatingStats("placeId", item.id);
      return {
        ...item,
