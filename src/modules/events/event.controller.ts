...
import { createEvent, getEventById, listAgenda, listEventsByPlace, updateEvent, listPendingEvents, moderateEvent } from "./event.service";
...
export const moderate = async (req: Request, res: Response) => {
  try {
    ...
    if (!eventStatus) throw new Error("Estado inválido");
    
    const result = await moderateEvent(String(req.params.id), eventStatus as any);
    res.json(result);
  } catch (error: any) {
