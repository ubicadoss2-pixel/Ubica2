...
export const callAI = async (
  ...
  const findPlaceInText = (text: string) => knownPlaces.find((p) => {
    if (text.includes(p.normalizedName)) return true;
    const words = p.normalizedName.split(' ');
    if (words.length > 1) {
      const partial = words.slice(0, 2).join(' ');
      if (partial.length > 4 && text.includes(partial)) return true;
    }
    return false;
  });
  ...
  const buildLocalAnswer = async (messagesArr: { role: string; content: string }[], normalizedMessage: string) => {
    // Other local intents...
    ...
    if (foodIntent.test(normalizedMessage)) {
      const restaurantSuggestions = knownPlaces.filter((p) =>
        /restaurante|cafe|gastrobar|bar|comida|cena|almuerzo/.test(p.type?.toLowerCase() || "") ||
    ...
    if (partyIntent.test(normalizedMessage)) {
      const nightlifeSuggestions = knownPlaces.filter((p) =>
        /bar|club|discoteca|coctel|rumba|noche/.test(p.type?.toLowerCase() || "") ||
    ...
    if (quietIntent.test(normalizedMessage)) {
      const quietSuggestions = knownPlaces.filter((p) =>
        /cafe|parque|museo|cultura/.test(p.type?.toLowerCase() || "") ||
  ...
  if (isExplicitRouteRequest) {
    try {
      ...
      let bestMatch: (typeof enriched)[number] | null =
        enriched
      ...
      if (!bestMatch && rawTarget.length >= 3) {
        ...
        for (const place of enriched) {
          ...
          const score = targetTokens.filter((t: string) => placeTokens.has(t)).length;
          if (score > bestScore) {
            bestScore = score;
            bestMatch = place;
          }
...
function formatEventSchedule(event: any): string {
  let scheduleStr = "";
...
async function buildDynamicSystemContext(): Promise<string> {
  try {
    const places = await prisma.place.findMany({
      where: { 
        status: "PUBLISHED", 
    ...
    places.forEach(p => {
      ...
      if (p.description) context += `  Descripción: ${p.description}\n`;
    });
    ...
    events.forEach(e => {
      ...
      if (e.description) context += `  Descripción: ${e.description}\n`;
...
export const sendMessage = async (
  ...
  const dbTimeout = (ms: number) => new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout")), ms));
  ...
  try {
    ...
    let tempConversation = await Promise.race([
      data.conversationId
        ? prisma.chatConversation.findUnique({ where: { id: data.conversationId } })
        : getOrCreateConversation(userId),
      dbTimeout(3000)
    ]);
      
    if (!tempConversation) {
      tempConversation = await getOrCreateConversation(userId);
    }
  ...
  try {
    if (conversation.id !== "offline-conv") {
      history = await getConversationMessages(conversation.id, 10);
    }
  } catch (e) {
  ...
  const aiResponse = await callAI(aiHistory, dynamicContext);

  console.log(`[CHATBOT] AI Response generated: ${aiResponse.content.substring(0, 50)}...`);
  
  const finalMetadata: any = aiResponse.metadata || {};
