...
export class ChatbotService {
  ...
  sendMessage(message: string, conversationId?: string, lat?: number, lng?: number): Observable<{ conversationId: string; message: ChatMessage }> {
    return this.http.post<{ conversationId: string; message: ChatMessage }>(
      `${this.baseUrl}/chatbot`,
      { message, conversationId, lat, lng }
    );
  }
