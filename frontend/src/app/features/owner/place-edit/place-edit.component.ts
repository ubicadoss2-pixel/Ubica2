...
export class PlaceEditComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.catalogsService.getCities().subscribe((items) => {
      const armeniaOnly = items.filter(c => c.name.toLowerCase() === 'armenia');
  ...
  submit(): void {
    ...
    this.placesService.update(this.placeId()!, payload).subscribe({
      next: (place) => {
        this.loading.set(false);
        this.notificationService.success('Éxito', '¡Perfecto! Los cambios han sido guardados exitosamente.');
      ...
      error: (err) => {
        this.loading.set(false);
        this.error.set('Error al actualizar el lugar. Inténtalo de nuevo.');
      },
    });
  }
}
