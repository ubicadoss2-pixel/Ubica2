...
const listPendingPlacesAdmin = async () => {
    return prisma_1.prisma.place.findMany({
        where: { status: "DRAFT", deletedAt: null },
        include: { city: true, ownerUser: true },
        orderBy: { createdAt: "desc" }
    });
};
exports.listPendingPlacesAdmin = listPendingPlacesAdmin;
const moderatePlaceAdmin = async (placeId, status, actorUserId, reason) => {
    const before = await prisma_1.prisma.place.findUnique({ where: { id: placeId } });
    if (!before)
        throw new Error("Negocio no encontrado");
    const place = await prisma_1.prisma.place.update({
        where: { id: placeId },
        data: { status }
    });
    await audit(actorUserId, `PLACE_MODERATION_${status}`, "PLACE", placeId, { ...before, moderationReason: reason }, place);
    return place;
};
exports.moderatePlaceAdmin = moderatePlaceAdmin;
const approveBusinessAdmin = async (placeId, actorUserId) => {
    return (0, exports.moderatePlaceAdmin)(placeId, "PUBLISHED", actorUserId);
};
...
const moderateEventAdmin = async (eventId, status, actorUserId, reason) => {
    const before = await prisma_1.prisma.event.findUnique({ where: { id: eventId } });
    if (!before)
        throw new Error("Evento no encontrado");
    ...
    await audit(actorUserId, `EVENT_MODERATION_${status}`, "EVENT", eventId, { ...before, moderationReason: reason }, event);
    return event;
};
exports.moderateEventAdmin = moderateEventAdmin;
const approveEventAdmin = async (eventId, actorUserId) => {
    return (0, exports.moderateEventAdmin)(eventId, "ACTIVE", actorUserId);
};
