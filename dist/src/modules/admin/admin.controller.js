...
const listPendingPlaces = async (_req, res) => {
    try {
        const places = await (0, admin_service_1.listPendingPlacesAdmin)();
        res.json(places);
    }
    catch (error) {
...
exports.listPendingPlaces = listPendingPlaces;
const moderatePlace = async (req, res) => {
    try {
        const { status, reason } = req.body;
        const result = await (0, admin_service_1.moderatePlaceAdmin)(req.params.id, status, req.user.id, reason);
...
exports.moderatePlace = moderatePlace;
const moderateEvent = async (req, res) => {
    try {
        const { status, reason } = req.body;
        const result = await (0, admin_service_1.moderateEventAdmin)(req.params.id, status, req.user.id, reason);
