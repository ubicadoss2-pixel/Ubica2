...
exports.moderate = exports.listPending = exports.agenda = exports.listByPlace = exports.getById = exports.update = exports.create = void 0;
...
const listPending = async (req, res) => {
    try {
        const events = await (0, event_service_1.listPendingEvents)();
        res.json(events);
    }
    catch (error) {
        res.status(500).json({ message: error.message });
...
const moderate = async (req, res) => {
    try {
        ...
        const eventStatus = status === "approved" ? "ACTIVE" : (status === "rejected" ? "REJECTED" : null);
        if (!eventStatus)
            throw new Error("Estado inválido");
        const result = await (0, event_service_1.moderateEvent)(String(req.params.id), eventStatus);
        res.json(result);
    }
    catch (error) {
