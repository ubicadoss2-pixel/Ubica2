...
exports.moderateEvent = exports.listPendingEvents = exports.listAgenda = exports.listEventsByPlace = exports.getEventById = exports.updateEvent = exports.createEvent = void 0;
...
const filterEventsByDate = (events, date, weekday, time) => {
    if (!date && weekday === undefined && !time)
        return events;
    const targetDate = date ? luxon_1.DateTime.fromISO(date, { zone: "utc" }) : null;
    const targetWeekday = weekday !== undefined ? weekday : (targetDate ? (targetDate.weekday === 7 ? 0 : targetDate.weekday) : undefined);
    return events.filter((event) => {
        let dateMatch = false;
        let timeMatch = true;
        if (time && event.startTime) {
            const eventTimeStr = luxon_1.DateTime.fromJSDate(event.startTime, { zone: "utc" }).toISOTime({ suppressMilliseconds: true, suppressSeconds: true })?.substring(0, 5) || "";
            if (time.length === 5) {
        ...
        if (date) {
            const hasOccurrence = occurrences.some((sd) => sd.dateType === "OCCURRENCE" && luxon_1.DateTime.fromJSDate(sd.eventDate, { zone: "utc" }).toISODate() === targetDate.toISODate());
            const hasException = occurrences.some((sd) => sd.dateType === "EXCEPTION" && luxon_1.DateTime.fromJSDate(sd.eventDate, { zone: "utc" }).toISODate() === targetDate.toISODate());
            if (hasOccurrence)
                dateMatch = true;
            ...
            const hasOccurrenceOnWeekday = occurrences.some((sd) => {
                if (sd.dateType !== "OCCURRENCE")
                    return false;
                const wd = luxon_1.DateTime.fromJSDate(sd.eventDate, { zone: "utc" }).weekday;
                const wk = wd === 7 ? 0 : wd;
                return wk === weekday;
            });
...
exports.listPendingEvents = listPendingEvents;
...
exports.moderateEvent = moderateEvent;
