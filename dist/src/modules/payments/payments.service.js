...
exports.processPaymentAndPromote = void 0;
const prisma_1 = require("../../config/prisma");
const processPaymentAndPromote = async (userId, targetType, targetId) => {
    const amount = 10000;
    // Verify target exists (relaxed ownership for demo — in production enforce ownerUserId check)
    if (targetType === "PLACE") {
        const place = await prisma_1.prisma.place.findUnique({ where: { id: targetId } });
        if (!place) {
    ...
    if (targetType === "PLACE") {
        await prisma_1.prisma.place.update({
            where: { id: targetId },
    ...
    const tx = await prisma_1.prisma.paymentTransaction.create({
        data: {
            userId,
            amount,
            targetType,
...
exports.processPaymentAndPromote = processPaymentAndPromote;
